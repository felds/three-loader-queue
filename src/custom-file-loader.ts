...
const loading: { [url: string]: { onLoad: Function; onProgress: Function; onError: Function }[] } = {}

class HttpError extends Error {
  public response: Response

  constructor(message: any, response: Response) {
    super(message)
    this.response = response
  }
}

class CustomFileLoader extends Loader {
        ...
        switch (responseType) {
          ...
          case "document":
            return response.text().then((text) => {
              ...
              const type = (
                typeof this.mimeType === "undefined" || !supportedTypes.includes(this.mimeType)
                  ? "text/html"
                  : this.mimeType
              ) as DOMParserSupportedType
